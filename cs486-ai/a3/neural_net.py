...
class NeuralNetwork():
    ...
    def forward_pass(self, X) -> (List[np.ndarray], List[np.ndarray]):
        ...
        first layer. Each layer computes the pre-activation z_i = [1, a_{i-1}] W[i] (with a_0 = X) and the activation
        a_i = g(z_i).
        :param X: The training set, with size (n, f)
        :return z_vals: a list of pre-activations for each example in the dataset. There are n_layers items in the list
                        and each item is an array of size (n, layer_sizes[i])
                a_vals: a list of activations for each example in the dataset. There are n_layers items in the list and
        ...
        for i in range(self.n_layers):
            ...
            aug = np.concatenate([ones, inp], axis=1)
            z = aug @ self.W[i]
    ...
    def backward_pass(self, z_vals, dLdyhat) -> List[np.ndarray]:
        '''
        ...
        :param z_vals: a list of pre-activations for each example in the dataset. There are n_layers items in the list
                       and each item is an array of size (n, layer_sizes[i])
        ...
        # output layer
        deltas[-1] = dLdyhat * self.activations[-1].derivative(z_vals[-1])
        # hidden layers
        for i in range(self.n_layers - 2, -1, -1):
            w_no_bias = self.W[i + 1][1:, :]
            deltas[i] = (deltas[i + 1] @ w_no_bias.T) * self.activations[i].derivative(z_vals[i])
        return deltas

    def update_weights(self, X, a_vals, deltas) -> List[np.ndarray]:
        '''
        ...
        :param X: The training set, with size (n, f)
        :param a_vals: a list of activations for each example in the dataset. There are n_layers items in the list and
                       each item is an array of size (n, layer_sizes[i])
    ...
    def train(self, X: np.ndarray, y: np.ndarray, epochs: int) -> (List[np.ndarray], List[float]):
        ...
        for epoch in range(epochs):
            z_vals, a_vals = self.forward_pass(X)   # Execute forward pass
            y_hat = a_vals[-1]                       # Get predictions
            ...
            deltas = self.backward_pass(z_vals, dLdyhat)     # Execute the backward pass to compute the deltas
            self.W = self.update_weights(X, a_vals, deltas)  # Calculate the gradients and update the weights
    ...
    def evaluate(self, X: np.ndarray, y: np.ndarray, metric) -> float:
        ...
        z_vals, a_vals = self.forward_pass(X)       # Make predictions for these examples
        y_hat = a_vals[-1]
